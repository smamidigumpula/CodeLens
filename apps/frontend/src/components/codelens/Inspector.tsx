...
export function Inspector({
  ...
  return (
    <aside className="flex min-h-0 flex-col gap-4 overflow-auto rounded-lg border border-border bg-card p-4 shadow-sm">
      <div>
        <p className="text-xs font-bold uppercase text-blue-600">Selected module</p>
        <h2 className="mt-1 break-words text-xl font-semibold">{file.name}</h2>
      ...
      <div className="min-h-0 overflow-hidden rounded-lg border border-border bg-slate-950">
        <div className="border-b border-white/10 px-3 py-2 text-xs font-bold text-slate-300">Preview</div>
        <pre className="max-h-64 overflow-auto p-3 text-xs leading-relaxed text-blue-100">{file.content || "No preview available."}</pre>
      </div>
    </aside>
  );
}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <span className="rounded-md border border-border bg-background p-2 text-xs text-muted-foreground">
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <button
            ...
            className="break-all rounded-md border border-border bg-background px-3 py-1.5 text-left text-xs hover:border-blue-300 hover:bg-blue-50"
          >
            {item}
          </button>
        ))
